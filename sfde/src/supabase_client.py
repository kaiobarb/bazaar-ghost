...
class SupabaseClient:
        ...
        try:
            ...
            if status == "completed":
                ...
                if current_chunk and current_chunk.data:
                    data["attempt_count"] = (
                        current_chunk.data.get("attempt_count", 0) or 0
                    ) + 1
    ...
    def update_chunk_progress(
        self, chunk_id: str, frames_processed: int, detections_count: int
    ):
    ...
    def upload_batch(self, matchups: List[Dict[str, Any]]):
        ...
        with create_span(
            "supabase_upload", attributes={"batch.size": len(matchups)}
        ) as span:
                    ...
                    for matchup in matchups:
                        ...
                        if source_id not in vod_id_cache:
                            try:
                                ...
                                self.logger.error(
                                    f"Failed to find VOD with source_id {source_id}: {e}"
                                )
                                continue
                        ...
                        if matchup.get("username"):
                            ...
                            if self.test_mode:
                                storage_path = (
                                    f"/detections/test/{self.quality}/{source_id}/{matchup['timestamp']}.jpg"
                                    if "frame_base64" in matchup
                                    else None
                                )
                            else:
                                storage_path = (
                                    f"/detections/{source_id}/{matchup['timestamp']}.jpg"
                                    if "frame_base64" in matchup
                                    else None
                                )
                            ...
                            self.logger.debug(
                                f"Skipping matchup at {matchup['timestamp']}: no valid username extracted"
                            )

                    # Insert detection records
                    if batch_data:
                        response = (
                            self.client.table("detections").insert(batch_data).execute()
                        )
                        self.logger.info(
                            f"Uploaded {len(batch_data)} detections to {self.schema} schema"
                        )
                        record_counter(
                            "detections_uploaded", len(batch_data), metric_attrs
                        )
        ...
        try:
            ...
            if self.test_mode:
                if image_type == "detection":
                    ...
                    filename = (
                        f"test/{self.quality}/{vod_id}/ocr_debug/boxes_{timestamp}.jpg"
                    )
                else:
    ...
    def delete_chunk_detections(self, chunk_id: str) -> int:
        ...
        try:
            ...
            self.logger.info(
                f"Found {len(detections)} existing detections for chunk {chunk_id}, deleting..."
            )
            ...
            for detection in detections:
                if detection.get("storage_path"):
                    try:
                        ...
                        self.logger.warning(
                            f"Failed to delete image {detection['storage_path']}: {e}"
                        )
