...
`;
    ...
    // Generate chunks for each VOD using the database function
    sql += "-- Generate chunks for all VODs\n";
    sql += "DO $$\n";
    sql += "DECLARE\n";
    sql += "  vod_record RECORD;\n";
    sql += "BEGIN\n";
    sql += "  FOR vod_record IN SELECT id, duration_seconds FROM public.vods LOOP\n";
    sql += "    PERFORM create_chunks_for_segment(vod_record.id, 0, vod_record.duration_seconds);\n";
    sql += "  END LOOP;\n";
    sql += "END $$;\n";
