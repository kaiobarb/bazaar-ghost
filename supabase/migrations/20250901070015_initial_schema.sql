...
-- Function to clean up chunks when VOD processing is disabled
CREATE OR REPLACE FUNCTION cleanup_vod_chunks_on_disable() RETURNS TRIGGER AS $$
BEGIN
...
-- Function to clean up chunks when streamer processing is disabled
CREATE OR REPLACE FUNCTION cleanup_streamer_chunks_on_disable() RETURNS TRIGGER AS $$
BEGIN
...
CREATE TRIGGER trigger_vod_processing_disabled
  ...
  FOR EACH ROW
  EXECUTE FUNCTION cleanup_vod_chunks_on_disable();

CREATE TRIGGER trigger_streamer_processing_disabled
  AFTER UPDATE OF processing_enabled ON streamers
  FOR EACH ROW
  EXECUTE FUNCTION cleanup_streamer_chunks_on_disable();
