-- Update process_pending_vods to exclude failed chunks from processing
...
BEGIN
    ...
    -- Find VODs with pending chunks (excluding failed chunks)
    FOR v_vod IN
        SELECT
            v.id AS vod_id,
            v.source_id,
...
-- Also update get_pending_chunks_for_vod to exclude failed chunks
...
BEGIN
    ...
    WHERE
        ...
        -- Only get chunks that are pending or queued (not processing, completed, or failed)
...
-- Update get_pending_vods_count to exclude failed chunks
...
-- Add comments
COMMENT ON FUNCTION process_pending_vods IS 'Selects up to N VODs with pending/queued chunks (excluding failed) and triggers processing via edge function';
COMMENT ON FUNCTION get_pending_chunks_for_vod IS 'Gets pending/queued chunks for a VOD (excluding failed chunks)';
COMMENT ON FUNCTION get_pending_vods_count IS 'Returns statistics about VODs with pending/queued chunks (excluding failed)';