...
def normalize_part(metadata: Dict[str, Any], cid: str, assume_bazaar: bool = False,
    ...
    if not (assume_bazaar or ranges or re.search(r'\bbazaar\b|大巴扎', title + ' ' + part_title, re.I)):
...
def ensure_account(owner: Dict[str, Any], enable: bool, profile_id: Optional[int],
                   streamer_id: Optional[int]) -> Dict[str, Any]:
    ...
    existing = api('platform_accounts', {'source': 'eq.bilibili', 'source_id': f'eq.{source_id}'})
    if existing:
