-- Restore the VOD-filtered search contract on environments retaining the older overload.
DROP FUNCTION IF EXISTS public.fuzzy_search_detections(text, bigint, text, double precision, integer, integer);
...
GRANT EXECUTE ON FUNCTION public.fuzzy_search_detections(text, bigint, text, text, double precision, integer, integer) TO anon, authenticated, service_role;
