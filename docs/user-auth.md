...
| Hosted expiry, suspension, and cleanup | An expired session and a suspended user's cookie fail both reads and writes. Reactivation does not revive revoked sessions. An actual hourly cleanup invocation removes bounded expired fixture state while preserving live state, including with outbound integrations disabled. |
