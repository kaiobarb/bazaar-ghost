...
| Hosted expiry and scheduled cleanup | An expired session fails both reads and writes. An actual hourly cleanup invocation removes bounded expired fixture state while preserving live state, including with outbound integrations disabled. Hosted suspension, immediate revocation, and reactivation without session revival already passed in the expanded social proof. |
