...
The shared admin retry endpoint is `/api/admin/retry-vod`, with the internal `vod_id` and optional `mode: "all"`; the default retries failed chunks while preserving completed siblings. **Validation build `1c5428c` still has a Twitch-only lookup defect in this endpoint.** The reviewed local correction uses the owning platform account's enablement and transactionally rejects competing disables or active workers. All eight new workerd cases and the full 173-test Worker suite pass; hosted validation and deployment remain pending. Do not treat the local change as an available hosted non-Twitch retry capability yet.
