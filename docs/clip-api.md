...
Clip/favorite cursors use clip IDs; comment cursors use insertion sequences. Visibility and ownership filters apply before the page limit. Traversal reads live rows: restoring or adding an older clip below the current cursor requires starting a new traversal to see it.
...
The hosted pagination/quota proof passed on the same build from `2026-09-08T00:27:43.647Z` to `00:29:07.723Z`. Five disposable clips had no detections or image objects. Alice's favorites returned `[49,51]` with cursor `51`, then `[52]` with a null cursor; hidden clip 50 was excluded, and Bob saw only his clip 53. Comment pages returned the first and third inserted comments, then the fifth, excluding a hidden middle comment and the suspended author's comment; the first cursor matched database sequence 11.

Exactly eight retries of one canonical report produced `200,200,200,200,200,429,200,200`. D1 minute `29813788` held a count of five; the actual next minute, `29813789`, reset to one and then incremented to two. One report and one report counter persisted throughout. No clock or counter was modified. This proves natural rollover of the shared quota mechanism through the report route; it does not claim a separate hosted rollover test for each category.

All fixture-owned rows were removed, foreign keys were clean, and all 43 original clip/detection full-row hashes were unchanged. Three genuine fixture moderation audits remain. No R2 objects were created. Evidence: `.ignore/platform-validation/hosted-social-pagination.7e89cbab-8d7e-42ec-a081-0d6a63505b73/proof.json`. Local integration tests additionally cover revocation/moderation races; real provider/browser acceptance remains in [user-auth.md](user-auth.md#verification-and-remaining-acceptance).
